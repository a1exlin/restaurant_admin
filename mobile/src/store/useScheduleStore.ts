...
function getDefaultState(): ScheduleState {
  ...
  try {
    ...
    if (saved) {
      ...
      return {
        ...
        roles: parsed.roles ?? ROLES.map((r) => ({ ...r, staff: [] })),
...
async function persistToStorage(state: ScheduleState): Promise<void> {
  try {
...
export function useScheduleStore() {
  const [state, setState] = useState<ScheduleState>(getDefaultState);
  ...
  const getHeadcount = useCallback(
    (roleId: RoleId, day: DayOfWeek): { morning: number; night: number } => {
      return state.headcounts[roleId]?.[day] ?? getDefaultHeadcount(roleId, day);
    },
    [state.headcounts]
  );
