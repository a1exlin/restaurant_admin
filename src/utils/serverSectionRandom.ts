...
/** Section letters used for server floor rotation (matches user input g,p,b,o,r,y → stored uppercase). */
...
/**
 * For `count` server assignments in one day, pick section letters so each of G,P,B,O,R,Y
 * appears floor(count/6) or ceil(count/6) times (fair split), in random order.
 */
...
/** Keep numeric time range if present; otherwise default opening shift with section suffix. */
